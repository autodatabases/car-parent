...
function bindEvent(){
	$("#typeName1").off("change").on("change",function(e){
		if($(e.target).val()!=""){
			if(($(e.target).find("option:selected").text()).indexOf("流量")!=-1||($(e.target).find("option:selected").text()).indexOf("话费")!=-1){
				$(".productAdd-form input[name='productId']").removeAttr("data-valide");
			}else{
				$(".productAdd-form input[name='productId']").attr("data-valide",true);
			}
...
function productAdd() {
    ...
    $.each(inputs, function (index, input) {
    	...
    	if($(input).attr("data-valide")&&$(input).val()==""){
    		$.tip(toastMessage.replace(/：/g, "")+"不能为空！");
    		$("#productAdd_button").prop("disabled", false).text("确定");
    		isValid = false;
            return false;
    	}else{
    		...
    		switch(valideName){
    			...
    			break;
    			case "productId":
	    			if($(input).attr("data-valide")&&!RegExpNum.test($(input).val())){
	    				$.tip(("请输入正确的"+toastMessage).replace(/：/g, ""));
				     	isValid = false;
	        			return false;
				    }    			
