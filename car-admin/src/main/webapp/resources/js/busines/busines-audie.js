...
$(function(){
	...
	$("#sellerGrade").on('change',function() {
		if ($(this).val() == "2")  {
			$(".serviceBrand").css("display","inline-block");
		} else {
			$(".serviceBrand").css("display","none");
	...
	$.ajax({
		...
		success : function(data, textStatus) {
			if (data.success) {
				...
				$("#companyName").val(dataseller.companyName);
				...
				if ($("#sellerGrade").val() == "2") {
					$("#serviceBrand").val(dataseller.carBrand);
					$(".serviceBrand").css("display","inline-block");
				}
...
/*				$("#scoreService option").each(function(i,e){
			    ...
			    if(data.data){
					...
					var sellerPolicy = JSON.parse(dataseller.sellerPolicy);
					$("#baoyangmoney").val(sellerPolicy.baoyangMoney);
					$("#penqimoney").val(sellerPolicy.penqiMoney);
					$("#baoyangcheck").prop('checked',sellerPolicy.baoyangCheck);
					$("#penqicheck").prop('checked',sellerPolicy.penqiCheck);
				}
			}else{
				$.tip("查询信息失败");
			}
...
function busines_button(){//提交
	var carBrand = "";
	if ($("#sellerGrade").val() == "2") {
		carBrand = $.trim($("#serviceBrand").val());
	}
    ...
    $.ajax({
		...
		data :{
			...
			baoyangmoney:$.trim($("#baoyangmoney").val()),
			penqimoney:$.trim($("#penqimoney").val()),
			baoyangcheck:$("#baoyangcheck").prop('checked'),
			penqicheck:$("#penqicheck").prop('checked')
		},
		traditional : true,// 使用传统方式序列化
		success : function(data, textStatus) {
			if (data.success) {
				$.tip("操作成功");
				setTimeout(function () {
					$('.tables').load('busines/busines-list.html?t=1');
				}, 2000);
			}else{
				$.tip(data.message);
				$("#busines_button").prop("disabled",false);
