...
public class OilController {
   ...
   */
   ...
   public DatasetSimple<Map<String,String>> openOilCard(HttpServletRequest request,
	   ...
	   return oilUserService.updateOilCard(startCode,endCode,province,address,
			   supplier,buyer,remark,proposer);
   }
   ...
   public DatasetList<Map<String, Object>> oilCheckList(HttpServletRequest request,
		   @Required Integer pageNo,@Required Integer pageSize,
		    ...
		    String num,String province,String address,String date,
   ...
   public DatasetList<Map<String,Object>> queryNewOilOrder(HttpServletRequest request
   		...
   		String userPhone,String orderNo,String orderType,String cardNo) {
       return oilUserService.queryNewOilOrder(Integer.valueOf(pageNo),
       		Integer.valueOf(pageSize),userPhone,orderNo,orderType,cardNo);
   }
   ...
   public void exportOilExcelTwo(
   		...
   		DatasetList<Map<String, String>> data = oilUserService.exportOilExcelTwo(date);
	   		...
	   		ExportExcelData.export(request, response, data.getList(), "油卡订单信息"+date, 
	   	   			new String[]{"服务商","订单号","订单类型","用户账号","充值卡号/充值手机号/卡号","充值金额","充值后余额",
	   	   					"订单状态","创建时间","接口提供者"},
	   	   			new String[]{"oilComp","orderNo","orderType","userPhone",
    ...
    * @param phone
    */
   @RequestMapping("exportOilExcelWithPhone")
   public void exportOilExcelWithPhone(
		   HttpServletRequest request,
		   HttpServletResponse response,
		   @Required @Regex("\\d+")String phone){
	   DatasetList<Map<String,String>> data = oilUserService.exportOilExcelWithPhone(phone);
	   ExportExcelData.export(request, response, data.getList(), "油卡订单信息"+phone, 
	   			new String[]{"服务商","订单号","订单类型","用户账号","充值卡号/充值手机号/卡号","充值金额","充值后余额",
	   					"订单状态","创建时间","接口提供者"},
	   			new String[]{"oilComp","orderNo","orderType","userPhone",
	   					"cardNo","money","surplusMoney","status","createTime","provider"},
   ...
   public void exportOilCardLiucun(HttpServletRequest request,
		   HttpServletResponse response,String date,String supplier,
		   String address,String money) {
   	DatasetList<Map<String, String>> data = oilUserService.exportOilCardLiucun(date,supplier,address,money);
