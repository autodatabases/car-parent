...
verification.dataList = function() {
	...
	$.ajax({
		...
		success : function(data, textStatus) {
			...
			$('#nuseNum').html(reTu.allAccountMoney);		
			if (data.success) {	
...
verification.renderHtml = function(datalist){
	...
	for(var i=0;i<datalist.length;i++){
		...
		html+='<td>'+datalist[i].accountMoney+'</td>';    
		html+="<td style='cursor:pointer' onclick=deletes('"+datalist[i].openDate+"','"+datalist[i].supplier+"','"+datalist[i].address+"','"+datalist[i].content+"')>导出</td>";          		          
		html+='</tr>';
...
function deletes(than,can,ress,mone){
	console.log(than)
	console.log(can)
	console.log(ress)
	console.log(mone)
	var dd=than.replace(/-/g, ",")
	var cc=dd.split(',')
	var ee='20'+cc[0]
	var ff=ee+'-'+cc[1]+'-'+cc[2]
	console.log(ff)
	window.open("oilCard/exportoilcardliucun?date="+ff+"&supplier="+can+"&address="+ress+"&money="+mone)
}
function initExportSelect() {
	...
	for (var i=0; i<12; i++) {
		if(year!==2018){
			if(month - i <= 0) {
				html += "<option value="+(month - i + 12)+" data-flag="+(month - i + 12)+">"+ (month - i + 12) + "月</option>";			
			} else {
				html += "<option value="+(month - i)+" data-flag="+(month - i)+">" + (month - i) + "月</option>";
			}
		}else{
			if(month - i <= 0&&month-i<3) {
				html += "<option value="+(month - i + 12)+" data-flag="+(month - i + 12)+">"+ (month - i + 12) + "月</option>";										
			} else if(month-i>3){
				html += "<option value="+(month - i)+" data-flag="+(month - i)+">" + (month - i) + "月</option>";
			}
