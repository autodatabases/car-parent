$(document).ready(function() {
	...
	if(type==1){//订单列表页跳转过来
		$.ajax({
			type:"post",
			url: DOMIN.MAIN + '/carwash/getCarWashByOrder',
			cache:false,
			async:true,
			dataType: "json",
			data : {
			...
			success: function(data) {
				if(data.success){
					...
					if(data.data.carWash.notifyTime!=null){
						$("#status").html("已使用");
		...
		$.ajax({
			type:"post",
			url: DOMIN.MAIN + '/carwash/sendCarWash',
			cache:false,
			async:true,
			dataType: "json",
			success: function(data) {
		...
		$(".back").click(function(){
			window.location.href=DOMIN.MAIN+"/carwash/carWash.html";
		});
