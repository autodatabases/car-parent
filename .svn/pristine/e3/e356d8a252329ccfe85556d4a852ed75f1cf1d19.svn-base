...
$(document).ready(function(){
	...
	$("#addFwb").click(function(){
		...
		new addDataBY($("#minValFwb").val(),$("#maxValFwb").val(),
				...
				$("#DpNum").val(),$("#coupon_20").val(),$("#coupon_50").val(),carType);
	});
	$("#ByNum").blur(function(){
...
addData.prototype = {
	...
	addData : function() {
		...
		if(this.type==1){//机油规则
			...
			this.data={ruleType:this.type,minimum:this.minValue,maximum:this.maxValue,ruleValue:this.by+","+this.pq+","+this.xc+","+this.dp+","+this.juan20+","+this.juan50,carType:this.carType};
 ...
 */
function addDataBY(minValue, maxValue, by, xc, pq, dp, juan20, juan50,carType) {
	this.data;
	this.type=0;
	this.minValue = minValue;
	this.maxValue = maxValue;
...
addDataBY.prototype.checkData = function() {
	if (!(/^[0-9]+$/.test(this.minValue)) || !(/^[0-9]+$/.test(this.maxValue))) {//数字都是数字
		...
		return false;
	}else if(!(/^[0-9]+$/.test(this.juan20))){
		$.tip("50或20元优惠券张数不对，请正确填写");
		return false;
	}else if(!(/^[0-9]+$/.test(this.juan50))){
		$.tip("50或20元优惠券张数不对，请正确填写");
		return false;
	}else if (this.by > 0) {
		if (this.carType == "") {
...
addDataBY.prototype.initElements = function() {
	...
	this.td7 = $("<td/>");
	this.td7.text(this.juan20);
	this.td8 = $("<td/>");
	this.td8.text(this.juan50);
	this.td9 = $("<td/>");
	this.delBtn = $("<span>x</span>");
	this.td9.append(this.delBtn);
	this.tr.append(this.td1).append(this.td2).append(this.td3)
	.append(this.td4).append(this.td5).append(this.td6).append(this.td7).append(this.td8).append(this.td9);
}
            	...
            	if(data.list!=null&&data.list.length>0){
            		...
            		for (var i = 0; i < data.list.length; i++) {
						if(data.list[i].ruleType==1){
							...
							if(val.length==4){
							 new addDataBY(data.list[i].minimum, data.list[i].maximum, val[0],val[2],val[1],val[3],0,0, data.list[i].carType);
							}else{
							 new addDataBY(data.list[i].minimum, data.list[i].maximum, val[0],val[2],val[1],val[3],val[4],val[5], data.list[i].carType);	
							}
						}else{
							//取消选中
